...
def calc_scaling(rir_ism, raw_stochastic, weighting_function, target_drr,
    ...
    resulting RIR will match the target DRR.
...
def calc_directivity_attenuation(delta_pos, azimuth_directivity,
    ...
    """
    Calculate attenuation caused by rotationons of the sources.
...
def _add_stochastic_rir(
        ...
        sound_speed, change_alpha, lower_limit_alpha
):
    """
    ...
    """
    # maximum peak of pra is shifted by 40 samples compared to physical
    # time of arrival (TOA)
    pra_toa_offset = 40
    ...
    # in some cases, the RIR of image source method may be already too diffuse
    # according to target DRR for the currently drawn alpha, so adding
    # additional reverberant parts causes an invalid solution
    # if change_alpha is set after some iterations of rejection sampling,
    # calculate alpha based on the early reflections of the RIR that will
    # guarantee a solution during scaling to match the target DRR,
    # but still matches the original range of alpha the best
    if change_alpha:
        ...
        if max_factor < alpha:
            # for current alpha, target DRR would be more direct as DRR of
            # early reflections, so re-draw alpha, that then creates a lower,
            # but achievable target DRR and therefore a solution during
            # later scaling can be found
            if max_factor < lower_limit_alpha:
                ...
                # sample alpha until valid solution found, that still lays
                # within the original range, but preserves convergence
                alpha = np.random.uniform(lower_limit_alpha, max_factor)
                while alpha > max_factor:
                    alpha = np.random.uniform(lower_limit_alpha, max_factor)
...
def calc_rirs(
    ...
    """
    Room impulse response (RIR) simulation for microphone pairs as described in
    'DIMINISHING DOMAIN MISMATCH FOR DNN-BASED ACOUSTIC DISTANCE ESTIMATION
    VIA STOCHASTIC ROOM REVERBERATION MODELS'
    ...
    Args:
        ...
        sensor_pos (array-like):
            ...
            reasonably spacing for the assumnptions to hold.
        ...
        directivity_factor_range (array-like):
            Range from which the directional factor of the sound source (alpha)
            is drawn uniformly
        ...
        kappa (float):
            Determines the fade-in of the stochastic part of the RIR.
            Lower values cause smoother transition.
        ...
        max_order_ism (int):
            Order of simulated image sources

        win_len_direct (int):
            One-sided length of the window defining the part of the RIR
            which belongs to the direect path.
        ...
        max_retries (int):
            Number of iterations to re-draw alpha factor if no solution could
            be found before maximum value possible for the setup is calculated
    ...
    while (np.abs(alpha_values[0] - alpha_values[1]) > alpha_diff
        ...
        for ch, rir_ism in enumerate(rirs_ism):
            ...
            # For larger distances between the microphones,
            # alpha may not need to be overwritten.
            rir_result, alpha = _add_stochastic_rir(
                ...
                sample_rate, sound_speed, change_alpha,
